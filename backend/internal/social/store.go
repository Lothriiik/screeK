...
func (s *Store) GetFollowingFeed(ctx context.Context, userID uuid.UUID, cursorID uint, limit int) ([]Post, error) {
	var posts []Post
	followingSubquery := s.db.WithContext(ctx).Model(&Follow{}).Select("followee_id").Where("follower_id = ?", userID)
	query := s.db.WithContext(ctx).Model(&Post{}).Where("user_id IN (?) OR user_id = ?", followingSubquery, userID)
	if cursorID > 0 {
		query = query.Where("id < ?", cursorID)
	}
	err := query.Preload("User").Order("id DESC").Limit(limit).Find(&posts).Error
...
func (s *Store) ToggleLike(ctx context.Context, userID uuid.UUID, postID uint) (bool, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			newLike := PostLike{PostID: postID, UserID: userID}
			res := tx.Clauses(clause.OnConflict{DoNothing: true}).Create(&newLike)
			if res.RowsAffected > 0 {
				tx.Model(&Post{}).Where("id = ?", postID).UpdateColumn("likes_count", gorm.Expr("likes_count + 1"))
				isLiked = true
			}
			return res.Error
		}
